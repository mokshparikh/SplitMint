...
const Expenses = ({ group, user, setView, setExpenses: setAppExpenses }) => {
  ...
  useEffect(() => {
    if (!amount || Number(amount) <= 0 || selectedParticipants.length === 0)
      return;
  ...
  const filteredExpenses = useMemo(() => {
    return expenses.filter((exp) => {
      ...
      const matchesParticipant =
        filterParticipant === "all" || exp.payerId === filterParticipant;
      ...
      return (
        matchesSearch && matchesParticipant && matchesDate && matchesAmount
      );
    });
  }, [
    expenses,
  ...
  const handleSubmit = async (e) => {
    ...
    if (!title || numAmount <= 0 || selectedParticipants.length === 0)
    ...
    const finalSplits =
      splitMode === "percentage"
        ? splits.map((s) => ({
            ...
            amount: Number(((numAmount * s.percentage) / 100).toFixed(2)),
          }))
        : splits;
  ...
  const handleEdit = (exp) => {
    ...
    setSplits(exp.splits);
    setSelectedParticipants(exp.splits.map((s) => s.participantId));
          ...
          <div style={styles.formSide}>
            <div style={styles.glassCard}>
              <h3 style={styles.cardHeader}>
                {editingId ? "✏️ Edit Bill" : "➕ New Bill"}
              </h3>
              <form onSubmit={handleSubmit} style={styles.formLayout}>
                  ...
                  <div style={styles.chipGrid}>
                    {participants.map((p) => (
                      <button
                        ...
                        style={{
                          ...styles.chip,
                          background: selectedParticipants.includes(p.id)
                            ? "#6366f1"
                            : "rgba(255,255,255,0.05)",
                          border: selectedParticipants.includes(p.id)
                            ? "1px solid #818cf8"
                    ...
                    {splits.map((s) => (
                      <div key={s.participantId} style={styles.splitRow}>
                        ...
                        <input
                          ...
                          type="number"
                          value={
                            splitMode === "percentage" ? s.percentage : s.amount
                          }
                          onChange={(e) =>
                            setSplits((prev) =>
                              prev.map((x) =>
                                x.participantId === s.participantId
                                  ? splitMode === "percentage"
                                    ? {
                                        ...x,
                ...
                <div style={styles.formFooter}>
                  <button type="submit" style={styles.saveBtn}>
                    {editingId ? "Save Changes" : "Post Expense"}
                  </button>
                  {editingId && (
                    <button
                      type="button"
                ...
                <div key={exp.id} style={styles.historyCard}>
                  ...
                  <div style={styles.historyBottom}>
                    <span style={styles.expMeta}>
                      {exp.date} •{" "}
                      {participants.find((p) => p.id === exp.payerId)?.name ||
                        "Member"}{" "}
                      paid
                    </span>
